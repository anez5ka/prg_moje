    ...
    internal class Program
    {
        ...
        class Person
        {
...

            {
                ...
                string filePath = "U:\\prg\\prg_moje\\Files\\" + Name + ".txt";
            ...
            public static Person LoadFromTxt()
            {
                ...
                while (true)
                {
                Console.Write("Load which person? (Write name of the person): ");
                string inputName = Console.ReadLine();
                string filePath = "U:\\prg\\prg_moje\\Files\\" + inputName + ".txt";
                    if (File.Exists(filePath))
                {
                    using (StreamReader reader = new StreamReader(filePath))
                    {
                            outName = reader.ReadLine();
                    }
                    return new Person(outName, 100,100); // Prepiste tento radek tak, ze v nem budete vracet vytvorenou instanci Person (osobu nactenou ze souboru)
                }
                }
